...
function isRateLimited(req) {
  ...
  const key = `${ip}:userRegisterProxy`;
  const now = Date.now();
...
exports.userRegisterProxy = onRequest(async (req, res) => {
  allowCORS(req, res);
  ...
  try {
    ...
    const {data} = await http.post("register/", req.body || {});
    console.log("userRegisterProxy", req.method, req.path, 201);
    res.status(201).send(data);
  } catch (err) {
    const code = err?.response?.status || 500;
    console.error("userRegisterProxy error", code, err?.message);
    res.status(code).send({ error: "register_proxy_error" });
  }
});