...
export default function AdminSidebar() {
  ...
  const itemClass = (path) => `block px-3 py-2 rounded ${loc.pathname.startsWith(path) ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100'}`;

  if (role !== 'admin') return null;

  if (collapsed) {
    return (
      <button onClick={toggle} title="Abrir panel" className="fixed left-3 top-20 z-40 px-3 py-2 bg-white border border-gray-200 rounded shadow hover:bg-gray-50">☰ Panel</button>
    );
  }
