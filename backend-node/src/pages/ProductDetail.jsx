    ...
    <Layout>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          ...
          <div className="flex gap-2 mt-4">{images.slice(1, 6).map((img) => (<img key={img.id} src={img.url} className="w-16 h-16 object-cover rounded" />))}</div>
        </div>
        <div>
          ...
          {variants.length > 0 && (
            <div className="mb-4"><label className="block text-sm font-medium text-gray-700 mb-2">Variante</label><select value={selectedVariantId ?? ''} onChange={(e) => setSelectedVariantId(e.target.value ? Number(e.target.value) : null)} className="w-full px-3 py-2 border border-gray-300 rounded"><option value="">Seleccionar</option>{variants.map((v) => (<option key={v.id} value={v.id}>{v.color || 'Color'} / {v.storage || 'Opciones'} — ${formatMoney(Number(v.price))}</option>))}</select></div>
          )}
