  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3"><Link to="/admin/producto/nuevo" className="px-4 py-2 bg-blue-600 text-white rounded">Nuevo producto</Link></div>
      </div>
      ...
      <div className="grid gap-3">
        {items.map(p => (
          <div key={p.id} className="p-4 bg-white border rounded flex items-center justify-between">
            ...
            <div className="flex gap-2"><Link to={`/admin/producto/${p.id}`} className="px-3 py-2 bg-gray-200 rounded">Editar</Link><button onClick={() => setToDelete(p)} className="px-3 py-2 bg-red-600 text-white rounded">Eliminar</button></div>
          </div>
        ))}
