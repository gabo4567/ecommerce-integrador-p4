...
const Checkout: React.FC = () => {
  ...
  const generateInvoice = async () => {
    ...
    try {
      ...
      const inv = await api.post<any>("invoices/", { order: orderId, number, issue_date: new Date().toISOString().slice(0,10), total: Number(total.toFixed(2)), payment_method: paymentMethod, status: "pending" });
            ...
            {activeStep === 3 && (
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="flex space-x-4">
                  ...
                  <button onClick={generateInvoice} className="flex-1 bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700">
                    Generar Factura
                  </button>
                </div>
                {(invoiceMessage || invoiceNumber) && (
                  <div className="mt-4 text-center">
                    ...
                    {invoiceNumber && <div className="text-sm font-medium text-gray-800">N° Factura: {invoiceNumber}</div>}
                  </div>
                )}
                <div className="mt-4">
                  <button disabled={confirmDisabled} onClick={createShipment} className={`w-full py-3 rounded-lg font-semibold ${confirmDisabled ? 'bg-gray-300 text-gray-600 cursor-not-allowed' : 'bg-purple-600 text-white hover:bg-purple-700'}`}>
                    Confirmar compra y generar Tracking
                  </button>
                </div>
              </div>
            )}
